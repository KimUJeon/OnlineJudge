...
<해설>
처음 받는 문자열을 기준 문자열로 지정하기 위해 a 를 list 형태로 입력받고 a_len 에 a 리스트의 길이를 저장시킨뒤
테스트케이스보다 한개가 적은 수를 반복시키는 반복문을 작성해 계속 비교할 문자열 b를 입력받는다.
입력받는 문자열들은 길이가 모두 같기 때문에 a 문자열과 b 문자열의 j 번째 인덱스를 비교해 다를경우 ?를 삽입하여
리스트를 문자열로 합쳐 출력시키는 join을 이용해 출력하였다.
